...
components/                 # UI compartida
  ...
  billing-estimate-card.tsx # cards de estimación de facturación. DOS modos: interno (/billing-tracker?tab=estimates, sin projectionsById) = card DETALLADA (media/fees/bruto/facturado/falta + card de accuracy mes previo). Portal/cliente (con projectionsById) = card SIMPLE (`SimpleMonthCard`): un número por proyecto — mes cerrado → FACTURADO real, mes en curso/próximo → ESTIMADO = (total facturable − ya facturado) ÷ meses restantes (remaining-forward, vía getClientBillingProjections); media/fees quedan en el desplegable por plan. Sin la jerga interna ni la card de variación
